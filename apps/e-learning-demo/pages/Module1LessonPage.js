...
class Module1LessonPage extends BaseLessonPage {
    ...
    renderContentSections() {
        ...
        const theorieTitle = (theorie && theorie.title) ? theorie.title : 'Het Framework Begrijpen';
    ...
    async afterContentLoaded() {
        ...
        if (!this.content || !this.contentLoaded) {
            console.error('[Module1LessonPage] ❌ Content not loaded properly');
            return false; // Stop initialization
        }
        
        // Check of theorie sectie bestaat
        if (!this.content.theorie) {
            console.warn('[Module1LessonPage] ⚠️ Theorie section missing in content');
        }
...
try {
    if (typeof module !== 'undefined' && module.exports) {
        module.exports = Module1LessonPage;
    } else {
        window.Module1LessonPage = Module1LessonPage;
    }
    console.log('[Module1LessonPage] ✅ Exported to window');
} catch (error) {
    console.error('[Module1LessonPage] ❌ Error exporting:', error);
    try {
        window.Module1LessonPage = Module1LessonPage;
    } catch (e) {
        console.error('[Module1LessonPage] ❌ Failed to force export:', e);
    }
}
