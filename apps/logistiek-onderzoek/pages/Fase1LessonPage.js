 ...
 * Fase1LessonPage
 *
 * Fase 01 – Eerste aanzet (fase1.html): probleem, doelstelling, onderzoeksmodel, hoofdvraag
 */

class Fase1LessonPage extends BaseLessonPage {
    constructor() {
        super('fase-1', 'Fase 01', 'Eerste aanzet');
    ...
    async generateMCQuestions() {
        ...
        try {
            ...
            console.log('[Week2LessonPage] Calling AIGenerator.generateMCQuestions...');
            const generatedQuestions = await this.aiGenerator.generateMCQuestions(theorySegment, 1, nextSegmentIndex);
            console.log('[Week2LessonPage] ✅ Generated question received:', generatedQuestions);
            ...
            console.error('[Week2LessonPage] ❌ Error generating MC questions:', error);
            console.error('[Week2LessonPage] Error details:', {
                ...
                <div class="border border-red-200 dark:border-red-800 rounded-lg p-5 bg-red-50 dark:bg-red-900/20">
                    <div class="flex items-start space-x-3">
                        ...
                        <div class="flex-1">
                            ...
                            <p class="text-red-800 dark:text-red-300 text-sm mb-3">${errorMessage}</p>
                            <div class="flex space-x-2">
                                <button 
    ...
    setupMCQuestionButton(retries = 5) {
        ...
        if (generateBtn) {
            ...
            if (generateBtn.dataset.listenerAttached === 'true') {
                console.log('[Week2LessonPage] MC question button already has listener');
                return;
            }
            ...
            console.log('[Week2LessonPage] ✅ MC question button setup complete');
        } else if (retries > 0) {
            // Retry if button not found yet (DOM might still be updating)
            console.log(`[Week2LessonPage] ⏳ MC question button not found, retrying... (${retries} attempts left)`);
            setTimeout(() => {
                this.setupMCQuestionButton(retries - 1);
            }, 100);
        } else {
            console.warn('[Week2LessonPage] ⚠️ MC question button not found in DOM after all retries');
        }
    }
...
try {
    if (typeof module !== 'undefined' && module.exports) {
        module.exports = Fase1LessonPage;
    } else {
        window.Fase1LessonPage = Fase1LessonPage;
    }
} catch (error) {
    console.error('[Fase1LessonPage] Error exporting:', error);
    try {
        window.Fase1LessonPage = Fase1LessonPage;
    } catch (e) {
        console.error('[Fase1LessonPage] Failed to force export:', e);
    }
}
