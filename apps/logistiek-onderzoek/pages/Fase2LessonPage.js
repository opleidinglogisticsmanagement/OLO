 ...
 * Fase2LessonPage
 *
 * Fase 02 – De verfijning (fase2.html): definiëren van begrippen, deelvragen
 */

class Fase2LessonPage extends BaseLessonPage {
    constructor() {
        super('fase-2', 'Fase 02', 'De verfijning');
    ...
    renderContentSections() {
        ...
        if (this.content.leerdoelen.interactive && this.content.leerdoelen.items && this.content.leerdoelen.items.length > 0) {
            ...
            leerdoelenContent = ContentRenderer.renderLearningObjectivesChecklist({
                ...
                storageKey: 'fase2-learning-objectives'
            });
        } else {
            // Use standard list
            leerdoelenContent = `
...
try {
    if (typeof module !== 'undefined' && module.exports) {
        module.exports = Fase2LessonPage;
    } else {
        window.Fase2LessonPage = Fase2LessonPage;
    }
} catch (error) {
    console.error('[Fase2LessonPage] Error exporting:', error);
    try {
        window.Fase2LessonPage = Fase2LessonPage;
    } catch (e) {
        console.error('[Fase2LessonPage] Failed to force export:', e);
    }
}
