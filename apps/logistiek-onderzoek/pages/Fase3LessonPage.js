 ...
 * Fase3LessonPage
 *
 * Fase 03 – Literatuuronderzoek (fase3.html): literatuuronderzoek, selecteren, bronnen, theoretisch kader
 */

class Fase3LessonPage extends BaseLessonPage {
    constructor() {
        super('fase-3', 'Fase 03', 'Literatuuronderzoek');
    ...
    renderContentSections() {
        ...
        if (this.content.leerdoelen.interactive && this.content.leerdoelen.items && this.content.leerdoelen.items.length > 0) {
            ...
            leerdoelenContent = ContentRenderer.renderLearningObjectivesChecklist({
                ...
                storageKey: 'fase3-learning-objectives'
            });
        } else {
            // Use standard list
            leerdoelenContent = `
    ...
    async generateMCQuestions() {
        ...
        try {
            ...
            if (generatedQuestions && generatedQuestions.length > 0) {
                ...
                // OPTIE 4: Prefetch next question in background (only after first question is shown)
    ...
    setupMCQuestionButton(retries = 5) {
        ...
        if (generateBtn) {
            ...
            console.log('[Week5LessonPage] ✅ MC question button setup complete');
        } else if (retries > 0) {
            // Retry if button not found yet (DOM might still be updating)
            console.log(`[Week5LessonPage] ⏳ MC question button not found, retrying... (${retries} attempts left)`);
            setTimeout(() => {
                this.setupMCQuestionButton(retries - 1);
            }, 100);
        } else {
            console.warn('[Week5LessonPage] ⚠️ MC question button not found in DOM after all retries');
        }
    }
