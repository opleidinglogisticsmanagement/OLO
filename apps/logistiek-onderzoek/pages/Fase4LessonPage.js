 ...
 * Fase4LessonPage
 *
 * Fase 04 – Onderzoekstechnisch ontwerp (fase4.html): kernbeslissingen, strategie, dataverzamelingsplan
 */

class Fase4LessonPage extends BaseLessonPage {
    constructor() {
        super('fase-4', 'Fase 04', 'Onderzoekstechnisch ontwerp');
    ...
    renderContentSections() {
        ...
        if (this.content.leerdoelen.interactive && this.content.leerdoelen.items && this.content.leerdoelen.items.length > 0) {
            ...
            leerdoelenContent = ContentRenderer.renderLearningObjectivesChecklist({
                ...
                storageKey: 'fase4-learning-objectives'
            });
        } else {
            // Use standard list
            leerdoelenContent = `
    ...
    async generateMCQuestions() {
        ...
        try {
            ...
            if (!this.cachedTheoryContent) {
                console.log('[Week6LessonPage] Extracting theory content (first time)...');
                this.cachedTheoryContent = this.aiGenerator.extractTheoryText(this.content.theorie.content || []);
                console.log('[Week6LessonPage] Cached theory content length:', this.cachedTheoryContent?.length || 0);
            } else {
                console.log('[Week6LessonPage] Using cached theory content');
            }
            ...
            if (this.totalSegments === null) {
                ...
                console.log('[Week6LessonPage] Total segments available:', this.totalSegments);
            }
            ...
            console.log('[Week6LessonPage] Using segment index:', nextSegmentIndex);
            ...
            console.log('[Week6LessonPage] Theory segment length:', theorySegment.length);
            console.log('[Week6LessonPage] Theory segment preview:', theorySegment.substring(0, 100) + '...');
            ...
            if (generatedQuestions && generatedQuestions.length > 0) {
                ...
                console.log('[Week6LessonPage] Used segments (last', maxTrackedSegments + '):', this.usedSegments);
    ...
    async prefetchNextQuestion() {
        ...
        try {
            ...
            console.log('[Week6LessonPage] Prefetching next question in background...');
            ...
            if (generatedQuestions && generatedQuestions.length > 0) {
                ...
                console.log('[Week6LessonPage] ✅ Next question prefetched successfully (segment:', nextSegmentIndex + ')');
            }
        } catch (error) {
            console.warn('[Week6LessonPage] Prefetch failed (will generate on-demand):', error.message);
    ...
    async loadNextQuestion() {
        ...
        try {
            ...
            if (this.prefetchedQuestion) {
                console.log('[Week6LessonPage] ✅ Using prefetched question (instant!)');
            ...
            console.log('[Week6LessonPage] No prefetched question, generating on-demand...');
            ...
            if (!this.cachedTheoryContent) {
                console.log('[Week6LessonPage] Extracting theory content for next question...');
                this.cachedTheoryContent = this.aiGenerator.extractTheoryText(this.content.theorie.content || []);
            }
            ...
            if (generatedQuestions && generatedQuestions.length > 0) {
                ...
                console.log('[Week6LessonPage] Used segments (last 3):', this.usedSegments);
    ...
    setupMCQuestionButton(retries = 5) {
        ...
        if (generateBtn) {
            ...
            console.log('[Week6LessonPage] ✅ MC question button setup complete');
        } else if (retries > 0) {
            // Retry if button not found yet (DOM might still be updating)
            console.log(`[Week6LessonPage] ⏳ MC question button not found, retrying... (${retries} attempts left)`);
            setTimeout(() => {
                this.setupMCQuestionButton(retries - 1);
            }, 100);
        } else {
            console.warn('[Week6LessonPage] ⚠️ MC question button not found in DOM after all retries');
        }
    }
...
try {
    if (typeof module !== 'undefined' && module.exports) {
        module.exports = Fase4LessonPage;
    } else {
        window.Fase4LessonPage = Fase4LessonPage;
    }
} catch (error) {
    console.error('[Fase4LessonPage] Error exporting:', error);
    try {
        window.Fase4LessonPage = Fase4LessonPage;
    } catch (e) {
        console.error('[Fase4LessonPage] Failed to force export:', e);
    }
}
