/**
 * CapaciteitsmanagementDeel1LessonPage
 * 
 * Specifieke pagina voor Capaciteitsmanagement (deel 1)
 */

class CapaciteitsmanagementDeel1LessonPage extends BaseLessonPage {
    constructor() {
        super('capaciteitsmanagement-deel1', 'Capaciteitsmanagement (deel 1)', 'Capaciteitsmanagement (deel 1)');
    }
        ...
        if (this.content.leerdoelen) {
            ...
            if (this.content.leerdoelen.interactive && this.content.leerdoelen.items && this.content.leerdoelen.items.length > 0) {
                leerdoelenContent = ContentRenderer.renderLearningObjectivesChecklist({
                    ...
                    storageKey: 'capaciteitsmanagement-deel1-learning-objectives'
                });
            } else {
                leerdoelenContent = `
    ...
    async afterContentLoaded() {
        if (!this.content || !this.contentLoaded) {
            console.error('[CapaciteitsmanagementDeel1LessonPage] ❌ Content not loaded properly');
            return false;
        }
        
        if (!this.content.theorie) {
            console.warn('[CapaciteitsmanagementDeel1LessonPage] ⚠️ Theorie section missing in content');
        }
...
try {
    if (typeof module !== 'undefined' && module.exports) {
        module.exports = CapaciteitsmanagementDeel1LessonPage;
    } else {
        window.CapaciteitsmanagementDeel1LessonPage = CapaciteitsmanagementDeel1LessonPage;
    }
    console.log('[CapaciteitsmanagementDeel1LessonPage] ✅ Exported to window');
} catch (error) {
    console.error('[CapaciteitsmanagementDeel1LessonPage] ❌ Error exporting:', error);
    try {
        window.CapaciteitsmanagementDeel1LessonPage = CapaciteitsmanagementDeel1LessonPage;
    } catch (e) {
        console.error('[CapaciteitsmanagementDeel1LessonPage] ❌ Failed to force export:', e);
    }
}
