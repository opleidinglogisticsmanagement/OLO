/**
 * CapaciteitsmanagementDeel2LessonPage
 * 
 * Specifieke pagina voor Capaciteitsmanagement (deel 2)
 */

class CapaciteitsmanagementDeel2LessonPage extends BaseLessonPage {
    constructor() {
        super('capaciteitsmanagement-deel2', 'Capaciteitsmanagement (deel 2)', 'Capaciteitsmanagement (deel 2)');
    }
        ...
        if (this.content.leerdoelen) {
            ...
            if (this.content.leerdoelen.interactive && this.content.leerdoelen.items && this.content.leerdoelen.items.length > 0) {
                leerdoelenContent = ContentRenderer.renderLearningObjectivesChecklist({
                    ...
                    storageKey: 'capaciteitsmanagement-deel2-learning-objectives'
                });
            } else {
                leerdoelenContent = `
    ...
    async afterContentLoaded() {
        if (!this.content || !this.contentLoaded) {
            console.error('[CapaciteitsmanagementDeel2LessonPage] ❌ Content not loaded properly');
            return false;
        }
        
        if (!this.content.theorie) {
            console.warn('[CapaciteitsmanagementDeel2LessonPage] ⚠️ Theorie section missing in content');
        }
...
try {
    if (typeof module !== 'undefined' && module.exports) {
        module.exports = CapaciteitsmanagementDeel2LessonPage;
    } else {
        window.CapaciteitsmanagementDeel2LessonPage = CapaciteitsmanagementDeel2LessonPage;
    }
    console.log('[CapaciteitsmanagementDeel2LessonPage] ✅ Exported to window');
} catch (error) {
    console.error('[CapaciteitsmanagementDeel2LessonPage] ❌ Error exporting:', error);
    try {
        window.CapaciteitsmanagementDeel2LessonPage = CapaciteitsmanagementDeel2LessonPage;
    } catch (e) {
        console.error('[CapaciteitsmanagementDeel2LessonPage] ❌ Failed to force export:', e);
    }
}
