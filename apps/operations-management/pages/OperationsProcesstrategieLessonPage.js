/**
 * OperationsProcesstrategieLessonPage
 * 
 * Specifieke pagina voor Operations en processtrategie
 */

class OperationsProcesstrategieLessonPage extends BaseLessonPage {
    constructor() {
        super('operations-processtrategie', 'Operations en processtrategie', 'Operations en processtrategie');
    }
        ...
        if (this.content.leerdoelen) {
            ...
            if (this.content.leerdoelen.interactive && this.content.leerdoelen.items && this.content.leerdoelen.items.length > 0) {
                leerdoelenContent = ContentRenderer.renderLearningObjectivesChecklist({
                    ...
                    storageKey: 'operations-processtrategie-learning-objectives'
                });
            } else {
                leerdoelenContent = `
    ...
    async afterContentLoaded() {
        if (!this.content || !this.contentLoaded) {
            console.error('[OperationsProcesstrategieLessonPage] ❌ Content not loaded properly');
            return false;
        }
        
        if (!this.content.theorie) {
            console.warn('[OperationsProcesstrategieLessonPage] ⚠️ Theorie section missing in content');
        }
...
try {
    if (typeof module !== 'undefined' && module.exports) {
        module.exports = OperationsProcesstrategieLessonPage;
    } else {
        window.OperationsProcesstrategieLessonPage = OperationsProcesstrategieLessonPage;
    }
    console.log('[OperationsProcesstrategieLessonPage] ✅ Exported to window');
} catch (error) {
    console.error('[OperationsProcesstrategieLessonPage] ❌ Error exporting:', error);
    try {
        window.OperationsProcesstrategieLessonPage = OperationsProcesstrategieLessonPage;
    } catch (e) {
        console.error('[OperationsProcesstrategieLessonPage] ❌ Failed to force export:', e);
    }
}
