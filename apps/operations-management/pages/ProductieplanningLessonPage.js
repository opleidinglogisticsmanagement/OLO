/**
 * ProductieplanningLessonPage
 * 
 * Specifieke pagina voor Productieplanning
 */

class ProductieplanningLessonPage extends BaseLessonPage {
    constructor() {
        super('productieplanning', 'Productieplanning', 'Productieplanning');
    }
        ...
        if (this.content.leerdoelen) {
            ...
            if (this.content.leerdoelen.interactive && this.content.leerdoelen.items && this.content.leerdoelen.items.length > 0) {
                leerdoelenContent = ContentRenderer.renderLearningObjectivesChecklist({
                    ...
                    storageKey: 'productieplanning-learning-objectives'
                });
            } else {
                leerdoelenContent = `
    ...
    async afterContentLoaded() {
        if (!this.content || !this.contentLoaded) {
            console.error('[ProductieplanningLessonPage] ❌ Content not loaded properly');
            return false;
        }
        
        if (!this.content.theorie) {
            console.warn('[ProductieplanningLessonPage] ⚠️ Theorie section missing in content');
        }
...
try {
    if (typeof module !== 'undefined' && module.exports) {
        module.exports = ProductieplanningLessonPage;
    } else {
        window.ProductieplanningLessonPage = ProductieplanningLessonPage;
    }
    console.log('[ProductieplanningLessonPage] ✅ Exported to window');
} catch (error) {
    console.error('[ProductieplanningLessonPage] ❌ Error exporting:', error);
    try {
        window.ProductieplanningLessonPage = ProductieplanningLessonPage;
    } catch (e) {
        console.error('[ProductieplanningLessonPage] ❌ Failed to force export:', e);
    }
}
