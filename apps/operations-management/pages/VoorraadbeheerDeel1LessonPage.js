/**
 * VoorraadbeheerDeel1LessonPage
 * 
 * Specifieke pagina voor Voorraadbeheer (deel 1)
 */

class VoorraadbeheerDeel1LessonPage extends BaseLessonPage {
    constructor() {
        super('voorraadbeheer-deel1', 'Voorraadbeheer (deel 1)', 'Voorraadbeheer (deel 1)');
    }
        ...
        if (this.content.leerdoelen) {
            ...
            if (this.content.leerdoelen.interactive && this.content.leerdoelen.items && this.content.leerdoelen.items.length > 0) {
                leerdoelenContent = ContentRenderer.renderLearningObjectivesChecklist({
                    ...
                    storageKey: 'voorraadbeheer-deel1-learning-objectives'
                });
            } else {
                leerdoelenContent = `
    ...
    async afterContentLoaded() {
        if (!this.content || !this.contentLoaded) {
            console.error('[VoorraadbeheerDeel1LessonPage] ❌ Content not loaded properly');
            return false;
        }
        
        if (!this.content.theorie) {
            console.warn('[VoorraadbeheerDeel1LessonPage] ⚠️ Theorie section missing in content');
        }
...
try {
    if (typeof module !== 'undefined' && module.exports) {
        module.exports = VoorraadbeheerDeel1LessonPage;
    } else {
        window.VoorraadbeheerDeel1LessonPage = VoorraadbeheerDeel1LessonPage;
    }
    console.log('[VoorraadbeheerDeel1LessonPage] ✅ Exported to window');
} catch (error) {
    console.error('[VoorraadbeheerDeel1LessonPage] ❌ Error exporting:', error);
    try {
        window.VoorraadbeheerDeel1LessonPage = VoorraadbeheerDeel1LessonPage;
    } catch (e) {
        console.error('[VoorraadbeheerDeel1LessonPage] ❌ Failed to force export:', e);
    }
}
