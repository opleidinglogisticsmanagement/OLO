/**
 * VoorraadbeheerDeel3LessonPage
 * 
 * Specifieke pagina voor Voorraadbeheer (deel 3)
 */

class VoorraadbeheerDeel3LessonPage extends BaseLessonPage {
    constructor() {
        super('voorraadbeheer-deel3', 'Voorraadbeheer (deel 3)', 'Voorraadbeheer (deel 3)');
    }
        ...
        if (this.content.leerdoelen) {
            ...
            if (this.content.leerdoelen.interactive && this.content.leerdoelen.items && this.content.leerdoelen.items.length > 0) {
                leerdoelenContent = ContentRenderer.renderLearningObjectivesChecklist({
                    ...
                    storageKey: 'voorraadbeheer-deel3-learning-objectives'
                });
            } else {
                leerdoelenContent = `
    ...
    async afterContentLoaded() {
        if (!this.content || !this.contentLoaded) {
            console.error('[VoorraadbeheerDeel3LessonPage] ❌ Content not loaded properly');
            return false;
        }
        
        if (!this.content.theorie) {
            console.warn('[VoorraadbeheerDeel3LessonPage] ⚠️ Theorie section missing in content');
        }
...
try {
    if (typeof module !== 'undefined' && module.exports) {
        module.exports = VoorraadbeheerDeel3LessonPage;
    } else {
        window.VoorraadbeheerDeel3LessonPage = VoorraadbeheerDeel3LessonPage;
    }
    console.log('[VoorraadbeheerDeel3LessonPage] ✅ Exported to window');
} catch (error) {
    console.error('[VoorraadbeheerDeel3LessonPage] ❌ Error exporting:', error);
    try {
        window.VoorraadbeheerDeel3LessonPage = VoorraadbeheerDeel3LessonPage;
    } catch (e) {
        console.error('[VoorraadbeheerDeel3LessonPage] ❌ Failed to force export:', e);
    }
}
